...
APP="$ROOT/${BUNDLE_NAME}.app"
rm -rf "$APP"
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources" "$APP/Contents/Frameworks"

# Convert Icon.iconset (or Icon.icon) to Icon.icns if present (requires iconutil).
ICON_TARGET="$ROOT/Icon.icns"
if [[ -d "$ROOT/Icon.iconset" ]]; then
  iconutil --convert icns --output "$ICON_TARGET" "$ROOT/Icon.iconset"
